# React+Node+SQLite自动部署到ECS实战-weekly周报工具
...
weekly 是一个**多用户**的周报管理工具，目标不是做一个简单的 Markdown 编辑器，而是把「一周一篇」的写作节奏，连同待办、便签两类碎片信息，沉淀到一条按 ISO 周次组织、起点固定 2026 年的时间轴上，长期可回顾。

功能上是三个彼此独立、共用一个账号体系的模块：**周报**（一周一篇 Markdown）、**待办**（独立清单，可选标记所属周次）、**便签**（纯文本碎片，无完成态）。
...
**现象**：workflow 里最初写成 <span v-pre>`echo "${{ secrets.ECS_SSH_KEY }}" > key`</span>，SSH 报私钥无效。
