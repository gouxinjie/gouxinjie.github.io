...
## 三、 镜像（Image）与 容器（Container）的区别

如果用一句话概括它们的区别：**镜像就是“安装程序/代码类”，容器就是“正在运行的软件进程/对象”**。

| 维度 | Docker 镜像 (Image) | Docker 容器 (Container) |
| --- | --- | --- |
| **状态** | **静态的**（只读文件，不占用 CPU/内存） | **动态的**（正在运行的进程，占用 CPU/内存） |
| **可读写性** | **只读（Read-Only）**，永远无法被直接修改 | **可读可写（Read-Write）**，顶部有独占的可写层 |
| **生命周期** | 长期存在，除非手动 delete（\`docker rmi\`） | 临时的，可以随时创建、暂停、停止、销毁（\`docker rm\`） |
| **关系数** | 一个镜像可以衍生出无数个相同的容器 | 每个容器都必须基于某个具体的镜像启动 |


**为什么一个容器不能包含多个镜像？**

因为容器的启动逻辑是：以某个镜像作为基础根文件系统（rootfs），并在其最上方覆盖一层可写层（Container Layer）来运行进程。

如果尝试在一个容器里挂载两个不同的镜像，文件系统层级的叠加和启动命令（CMD / ENTRYPOINT）就会产生混乱。
`;export{n as default};
