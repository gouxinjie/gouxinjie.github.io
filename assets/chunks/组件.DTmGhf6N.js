...
通过一个共享服务，配合 \`RxJS\` 的 \`Subject\`/\`BehaviorSubject\` 作为「消息总线」，实现任意组件之间的通信：
...
| 方式             | 优点                       | 缺点                         | 适用场景               |
| ---------------- | -------------------------- | ---------------------------- | ---------------------- |
| 共享服务 + Subject | 解耦、灵活、任意组件可通信 | 需要手动管理订阅的生命周期   | 跨层级、无直接关系的组件 |
| 父组件中转       | 简单直观、数据流清晰       | 组件层级深时逐层传递较繁琐   | 相邻兄弟、层级简单的场景 |
`;export{n as default};
