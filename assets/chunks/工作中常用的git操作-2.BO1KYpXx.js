...
从远程仓库获取最新的提交记录和分支信息，但不会自动合并这些更改到本地分支; 它会更新本地的远程分支引用，但不会修改本地的工作目录或当前分支。

如果需要将这些更改合并到本地分支，需要手动执行 \`git merge\` 或 \`git rebase\`。

## 5. git 撤销已 push 分支的方式

**1.** \`git revert\`：创建一个新的提交来撤销之前的提交；

\`git revert\` 是一种安全的方式来撤销提交。**它会创建一个新的提交记录，这个提交的内容是撤销指定提交的更改。**
